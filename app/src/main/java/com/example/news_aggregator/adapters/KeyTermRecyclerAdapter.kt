...
class KeyTermRecyclerAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class KeyTermViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        private var ref = database.collection(itemView.context.getString(R.string.firestore_users)).document(mAuth.uid.toString())

        fun bind(keyTerm: String) {
            keyTermTextView.text = keyTerm
            deleteButton.setOnClickListener {
                ref.update(itemView.context.getString(R.string.firestore_key_terms), FieldValue.arrayRemove(keyTerm))
            }
        }
    }
}